      ...
      >
        ...
        <View style={styles.topBar}>
          <View style={styles.logoWrap}>
            <Image source={images.mascotLogo} style={styles.logoImg} resizeMode="contain" />
          </View>
        ...
        <View style={styles.greetingSection}>
          <View style={styles.greetingText}>
            ...
            <Text style={styles.greetName}>{firstName} <Text style={{ fontSize: 28 }}>👋</Text></Text>
            <Text style={styles.greetSub}>Ready for today's{'\n'}language mission?</Text>
          </View>
          ...
          <View style={styles.mascotCol}>
            ...
            <Image source={images.mascotWelcome} style={styles.mascotImg} resizeMode="contain" />
          ...
          >
            ...
            <View style={styles.heroRow}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.heroSub}>Real conversations. Real progress.</Text>

                <View style={styles.heroFeatures}>
                  <View style={styles.heroFeatureItem}>
                    <Ionicons name="color-wand" size={13} color="rgba(255,255,255,0.85)" />
                    <Text style={styles.heroFeatureText}>Real-time correction</Text>
                  </View>
                  <View style={styles.heroFeatureItem}>
                    <Ionicons name="pulse" size={13} color="rgba(255,255,255,0.85)" />
                    <Text style={styles.heroFeatureText}>Voice chat</Text>
                  </View>
                  <View style={styles.heroFeatureItem}>
                    <Ionicons name="stats-chart" size={13} color="rgba(255,255,255,0.85)" />
                    <Text style={styles.heroFeatureText}>Pronunciation feedback</Text>
                  </View>
                </View>
              </View>
        ...
        <View style={styles.section}>
          ...
          >
            <LessonCard flag="🇪🇸" lang="Spanish Basics" pct={72} mins={12} bg="#FFFBEB" trackBg="#FEF3C7" />
            <LessonCard flag="🇺🇸" lang="English Fluency" pct={56} mins={18} bg="#F3F0FF" trackBg="#E0D4FF" />
            <LessonCard flag="🇯🇵" lang="Japanese Speaking" pct={38} mins={25} bg="#FEF2F2" trackBg="#FEE2E2" />
          </ScrollView>
        </View>
        ...
        <View style={[styles.section, { paddingHorizontal: 20 }]}>
          ...
          <View style={styles.toolsRow}>
            <ToolCard icon={<MaterialCommunityIcons name="robot-outline" size={20} color={PURPLE} />} iconBg="#F3E8FF" title="AI Tutor" sub="Personalized lessons" />
            <ToolCard icon={<Ionicons name="pulse" size={20} color="#4F46E5" />} iconBg="#EEF2FF" title="Pronunciation Scanner" sub="Speak & improve" />
            <ToolCard icon={<Ionicons name="chatbubble-ellipses-outline" size={20} color="#10B981" />} iconBg="#ECFDF5" title="Live Conversation" sub="Talk with AI" />
          </View>

          {/* Row 2 */}
          <View style={[styles.toolsRow, { marginTop: 10 }]}>
            <ToolCard icon={<Ionicons name="book-outline" size={20} color="#22C55E" />} iconBg="#F0FDF4" title="Vocabulary Builder" sub="Expand words" />
            <ToolCard icon={<Ionicons name="pencil-outline" size={20} color={ORANGE} />} iconBg="#FFF7ED" title="Grammar Fixer" sub="Fix in one tap" />
            <ToolCard icon={<Ionicons name="mic-outline" size={20} color="#EF4444" />} iconBg="#FEF2F2" title="Voice Challenge" sub="Beat your best" />
          </View>
        </View>
        ...
        <View style={styles.motiBannerWrap}>
          <View style={styles.motiBanner}>
            ...
            <Image source={images.mascotAuth} style={styles.motiMascot} resizeMode="contain" />
          </View>
        </View>
      </ScrollView>
    </SafeAreaView>
...
function LessonCard({
  flag, lang, pct, mins, bg, trackBg,
}: {
  flag: string; lang: string; pct: number; mins: number; bg: string; trackBg: string;
}) {
  return (
    <TouchableOpacity activeOpacity={0.85} style={[styles.lessonCard, { backgroundColor: bg }]}>
      <View style={styles.lessonCardTop}>
        <View style={styles.flagCircle}>
          <Text style={{ fontSize: 14 }}>{flag}</Text>
        </View>
        <Text style={styles.lessonLang} numberOfLines={2}>{lang}</Text>
      </View>
...
function ToolCard({
  ...
  return (
    <TouchableOpacity activeOpacity={0.85} style={styles.toolCard}>
      <View style={[styles.toolIconWrap, { backgroundColor: iconBg }]}>{icon}</View>
      <Text style={styles.toolTitle} numberOfLines={2}>{title}</Text>
      <Text style={styles.toolSub} numberOfLines={1}>{sub}</Text>
    </TouchableOpacity>
  );
}
...
const styles = StyleSheet.create({
  ...
  appName: { fontSize: 20, fontWeight: '800', color: DARK, letterSpacing: -0.4 },
  ...
  notifDot: {
    ...
    borderWidth: 1.5, borderColor: '#F7F7FB',
  },
  ...
  greetHi: { fontSize: 22, fontWeight: '700', color: DARK, letterSpacing: -0.3 },
  greetName: { fontSize: 32, fontWeight: '900', color: PURPLE, letterSpacing: -0.5, marginTop: 2 },
  ...
  heroTitle: { color: '#fff', fontSize: 22, fontWeight: '900', letterSpacing: -0.3, marginBottom: 4 },
  ...
  heroFeatureText: { color: 'rgba(255,255,255,0.85)', fontSize: 11, fontWeight: '500' },
  heroCTA: {
    width: 46,
    height: 46,
    borderRadius: 23,
  ...
  statLabel: { fontSize: 9, fontWeight: '700', color: '#94A3B8', textTransform: 'uppercase', letterSpacing: 0.3, textAlign: 'center' },
  xpRing: {
    width: 54,
    height: 54,
    borderRadius: 27,
  ...
  sectionTitle: { fontSize: 18, fontWeight: '800', color: DARK, letterSpacing: -0.3 },
  ...
  lessonCardTop: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 14 },
  flagCircle: {
    width: 30,
    height: 30,
    borderRadius: 15,
  ...
  lessonLang: { flex: 1, fontSize: 12, fontWeight: '700', color: DARK, lineHeight: 16 },
  progressTrack: { height: 5, borderRadius: 3, marginBottom: 8, overflow: 'hidden' },
  progressFill: { height: '100%', backgroundColor: PURPLE, borderRadius: 3 },
  lessonCardBottom: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  ...
  toolTitle: { fontSize: 11, fontWeight: '700', color: DARK, lineHeight: 15, marginBottom: 2 },
  ...
  motiLine1: { fontSize: 17, fontWeight: '800', color: DARK, lineHeight: 26, letterSpacing: -0.2 },
  motiAccent: { fontSize: 17, fontWeight: '800', color: PURPLE, lineHeight: 26, letterSpacing: -0.2 },
